...
export function SortRuleRow({ rule, index, total, availableColumns, onChange, onDelete, onMoveUp, onMoveDown }: Props) {
  ...
  return (
    <div className="border border-dark-border rounded p-2 bg-dark-bg space-y-2">
      <div className="flex items-center gap-1 text-xs text-gray-400">
        ...
        <button
          ...
          disabled={index === 0}
          title={tr('上移', 'Move up')}
          className="p-1 rounded hover:bg-dark-surface disabled:opacity-30 disabled:hover:bg-transparent"
        >
      ...
      <div className="flex items-center gap-1 text-xs">
        <button
          ...
          className={`px-2 py-0.5 rounded border ${isCol ? 'border-blue-500/50 bg-blue-500/10 text-blue-200' : 'border-dark-border text-gray-400 hover:border-dark-border hover:bg-dark-surface'}`}
        >
          {tr('列', 'Column')}
        </button>
        <button
      ...
      {isCol ? (
        <select
          ...
          className="w-full bg-dark-panel border border-dark-border rounded px-2 py-1 text-xs text-gray-200 focus:outline-none focus:border-blue-500/50"
        >
        ...
        <div>
          <input
            ...
            className={`w-full bg-dark-panel border rounded px-2 py-1 text-xs text-gray-200 font-mono focus:outline-none ${exprError ? 'border-red-500/50' : 'border-dark-border focus:border-blue-500/50'}`}
          />
          {exprError && <div className="text-[11px] text-red-400 mt-1">{exprError}</div>}
        </div>
      )}
      ...
      <div className="flex items-center gap-1 text-xs">
        <button
          ...
          className={`flex items-center gap-1 px-2 py-0.5 rounded border ${!rule.desc ? 'border-blue-500/50 bg-blue-500/10 text-blue-200' : 'border-dark-border text-gray-400 hover:bg-dark-surface'}`}
        >
        ...
        <select
          ...
          className="bg-dark-panel border border-dark-border rounded px-1 py-0.5 text-[11px] text-gray-300 focus:outline-none focus:border-blue-500/50"
        >
