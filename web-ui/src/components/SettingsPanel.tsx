  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
      <motion.div
        ...
        className="bg-dark-panel border border-dark-border rounded-xl shadow-2xl w-[600px] overflow-hidden flex flex-col"
      >
              ...
              <div className="bg-dark-bg border border-dark-border rounded-lg p-4">
                ...
                {policy ? (
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-dark-panel border border-dark-border p-3 rounded">
                      <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-1">Direct Match Threshold</div>
                      <div className="text-lg font-mono text-green-400">{policy.rule_direct_threshold.toFixed(2)}</div>
                    </div>
                    <div className="bg-dark-panel border border-dark-border p-3 rounded">
                      <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-1">Suggest Match Threshold</div>
              ...
              <div className="bg-dark-bg border border-dark-border rounded-lg p-4">
                ...
                {!(isConfigLoading || isAiModelsLoading) && (
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-dark-panel border border-dark-border p-3 rounded">
                        <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-2">Active Profile</div>
                        <select
                          value={config?.active_ai_profile_id || ''}
                          onChange={(e) => setActiveProfileId(e.target.value)}
                          disabled={isLoading || !config}
                          className="w-full bg-dark-bg border border-dark-border rounded px-2 py-1.5 text-sm text-gray-200"
                        >
                      ...
                      <div className="bg-dark-panel border border-dark-border p-3 rounded">
                        <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-2">Model</div>
                        <select
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      <div className="bg-dark-panel border border-dark-border p-3 rounded">
                        <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-2">Tier</div>
                        <select
                          value={resolvedTier}
                          onChange={(e) => saveModelAndTier(resolvedModelId, e.target.value)}
                          disabled={isLoading || !supportsTier}
                          className="w-full bg-dark-bg border border-dark-border rounded px-2 py-1.5 text-sm text-gray-200 disabled:opacity-60"
                        >
                      ...
                      <div className="bg-dark-panel border border-dark-border p-3 rounded">
                        <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-2">Health Check</div>
                        <Button
                ...
                {config && (
                  <div className="space-y-2">
                    {(Array.isArray(config.ai_profiles) ? config.ai_profiles : []).map((p: any) => {
                      ...
                      return (
                        <div key={p.id} className={`flex items-center justify-between gap-3 bg-dark-panel border rounded-lg px-3 py-2 ${isActive ? 'border-blue-500/40' : 'border-dark-border'}`}>
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="text-xs px-2 py-1 rounded border border-dark-border bg-dark-bg text-gray-300 hover:text-white hover:bg-dark-surface disabled:opacity-50"
                            >
                              {isActive ? 'Active' : 'Use'}
                            </button>
                            <button
                              ...
                              disabled={isLoading}
                              className="p-1.5 rounded border border-dark-border bg-dark-bg text-gray-400 hover:text-blue-400 hover:bg-blue-500/10 transition-colors"
                              title="Edit"
                            >
                            ...
                            <button
                              ...
                              disabled={isLoading}
                              className="p-1.5 rounded border border-dark-border bg-dark-bg text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-colors"
                              title="Delete"
                            >
                ...
                <div className={`border rounded-lg p-4 ${healthReport ? 'bg-green-950/10 border-green-500/20' : 'bg-red-950/10 border-red-500/20'}`}>
                  ...
                  {healthReport && (
                    <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                      <div className="bg-dark-bg border border-dark-border rounded p-3">
                        <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-1">Endpoint</div>
                        <div className="text-gray-200 break-all">{healthReport.endpoint}</div>
                      </div>
                      <div className="bg-dark-bg border border-dark-border rounded p-3">
                        <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-1">Profile / Model / Tier</div>
                        <div className="text-gray-200 break-all">{healthReport.active_ai_profile_id || '-'} · {healthReport.model_id} · {healthReport.tier}</div>
                      </div>
                      <div className="bg-dark-bg border border-dark-border rounded p-3">
                        <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-1">Latency</div>
                        <div className="text-gray-200">{healthReport.latency_ms ?? '-'} ms</div>
                      </div>
                      <div className="bg-dark-bg border border-dark-border rounded p-3">
                        <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-1">Preview</div>
                  ...
                  {healthError && (
                    <div className="mt-3 text-sm text-gray-200">
                      <div className="bg-dark-bg border border-dark-border rounded p-3">
                        <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-1">Error</div>
                  ...
                  {healthSuggestions.length > 0 && (
                    <div className="mt-3 bg-dark-bg border border-dark-border rounded p-3">
                      <div className="text-[11px] text-gray-500 uppercase font-bold tracking-wider mb-2">建议</div>
          ...
          {activeTab === 'db' && (
            <div className="space-y-6">
              <div className="bg-dark-bg border border-dark-border rounded-lg p-4">
                ...
                <div className="mb-6 p-4 bg-dark-panel border border-dark-border rounded-lg space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full bg-dark-bg border border-dark-border rounded px-3 py-1.5 text-sm text-gray-200"
                      />
                    </div>
                    <div>
                ...
                {!config ? (
                  ...
                  <div className="space-y-3">
                    {(Array.isArray(config.db_connections) ? config.db_connections : []).map((conn: any, idx: number) => {
                      ...
                      return (
                        <div key={conn.id || idx} className={`flex flex-col gap-2 bg-dark-panel border rounded-lg p-3 ${isActive ? 'border-blue-500/40' : 'border-dark-border'}`}>
                          ...
                          <div className="flex items-center gap-2 mt-2 ml-6">
                            <label className="flex items-center gap-1.5 text-sm text-gray-300 cursor-pointer">
                              <input 
                                ...
                                className="rounded border-dark-border bg-dark-bg text-blue-500 focus:ring-blue-500/20"
                              />
                              Read-Only Mode
                            </label>
                          </div>
      ...
      {isProfileEditorOpen && profileDraft && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <motion.div
            ...
            className="bg-dark-panel border border-dark-border rounded-xl shadow-2xl w-[680px] overflow-hidden"
          >
            <div className="px-6 py-4 border-b border-dark-border flex items-center justify-between bg-dark-bg">
            ...
            <div className="p-6 space-y-4">
                  ...
                  <div className="mt-4 pt-4 border-t border-dark-border">
                    ...
                    <div className="flex gap-2 mb-2">
                      <select
                        ...
                        className="flex-1 bg-dark-bg border border-dark-border rounded px-3 py-1.5 text-sm text-gray-200"
                      >
                    ...
                    <textarea
                      ...
                      className="w-full h-24 bg-dark-bg border border-dark-border rounded px-3 py-2 text-sm text-gray-200 font-mono"
                    />
                    <div className="text-[11px] text-gray-500 mt-1">若填写，新添加的模型将绑定这些推理等级。支持字段: id, display_name, temperature, max_tokens, reasoning_effort, thinking_type, thinking_budget_tokens, thinking_display</div>
                  </div>
                </div>
              </div>
              ...
              <div>
                  ...
                  <textarea
                    ...
                    className="w-full h-28 bg-dark-bg border border-dark-border rounded px-3 py-2 text-sm text-gray-200 font-mono"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="bg-dark-bg border border-dark-border rounded p-2 max-h-32 overflow-y-auto space-y-1">
                    {fetchedModels.length > 0 ? fetchedModels.map(m => (
                      <div key={m.id} className="flex items-center justify-between text-sm py-1 border-b border-dark-border last:border-0">
                  ...
                  <div className="mt-2 flex gap-2">
                    <input 
                      ...
                      className="flex-1 bg-dark-bg border border-dark-border rounded px-3 py-1 text-sm text-gray-200"
                      onKeyDown={async (e) => {
                        if (e.key === 'Enter') {
            ...
            <div className="px-6 py-4 border-t border-dark-border bg-dark-bg flex items-center justify-end gap-3">
              <button
                onClick={() => { setIsProfileEditorOpen(false); setProfileDraft(null) }}
